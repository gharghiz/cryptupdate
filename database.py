...
def init_db():
    try:
        if USE_POSTGRES:
            ...
            for col in ["summary", "sentiment", "reason", "category", "image_url", "url"]:
                try:
            ...
            try:
                cur.execute("ALTER TABLE posted_news ADD COLUMN views INTEGER DEFAULT 0")
            except Exception:
                pass

            cur.execute("CREATE INDEX IF NOT EXISTS idx_posted_news_cat ON posted_news (category, posted_at DESC)")
            cur.execute("CREATE INDEX IF NOT EXISTS idx_posted_news_views ON posted_news (views DESC)")
            conn.commit()
            cur.close()
            conn.close()
        else:
            with get_sqlite_conn() as conn:
                conn.execute("""
                    CREATE TABLE IF NOT EXISTS posted_news (
                        ...
                        image_url TEXT, url TEXT, views INTEGER DEFAULT 0
                    )
                """)
                conn.execute("""
                    CREATE TABLE IF NOT EXISTS telegram_log (
                ...
                for col in ["summary", "sentiment", "reason", "category", "image_url", "url"]:
                    try:
                        ...
                        pass
                try:
                    conn.execute("ALTER TABLE posted_news ADD COLUMN views INTEGER DEFAULT 0")
                except Exception:
                    pass
                conn.execute("CREATE INDEX IF NOT EXISTS idx_posted_news_cat ON posted_news (category, posted_at DESC)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_posted_news_views ON posted_news (views DESC)")
                conn.commit()
...
def save_news(news_id: str, title: str, source: str,
    ...
    try:
        if USE_POSTGRES:
            conn = get_pg_conn(); cur = conn.cursor()
            cur.execute("""
                INSERT INTO posted_news (id, title, source, posted_at, summary, sentiment, reason, category, image_url, url)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                ON CONFLICT (id) DO NOTHING
            """, (news_id, title, source, posted_at, summary, sentiment, reason, category, image_url, url))
            conn.commit(); cur.close(); conn.close()
        else:
            with get_sqlite_conn() as conn:
                conn.execute("""
                    INSERT OR IGNORE INTO posted_news (id, title, source, posted_at, summary, sentiment, reason, category, image_url, url)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (news_id, title, source, posted_at, summary, sentiment, reason, category, image_url, url))
                conn.commit()
        cache_clear()
...
def increment_views(news_id: str):
    try:
        if USE_POSTGRES:
            conn = get_pg_conn(); cur = conn.cursor()
            cur.execute("UPDATE posted_news SET views = COALESCE(views, 0) + 1 WHERE id = %s", (news_id,))
            conn.commit(); cur.close(); conn.close()
        else:
            with get_sqlite_conn() as conn:
                conn.execute("UPDATE posted_news SET views = COALESCE(views, 0) + 1 WHERE id = ?", (news_id,))
                conn.commit()
        cache_clear()
    except Exception:
        pass
...
def get_trending_news(limit: int = 10) -> list:
    try:
        if USE_POSTGRES:
            conn = get_pg_conn()
            ...
            cur.execute("""
                ...
                WHERE posted_at > %s AND views > 0
                ORDER BY views DESC, posted_at DESC
                LIMIT %s
            """, ((now_utc() - timedelta(hours=48)).isoformat(), limit))
            rows = cur.fetchall()
            cur.close(); conn.close()
            ...
            with get_sqlite_conn() as conn:
                rows = conn.execute("""
                    SELECT * FROM posted_news
                    WHERE posted_at > datetime('now', '-48 hours') AND views > 0
                    ORDER BY views DESC, posted_at DESC
                    LIMIT ?
                """, (limit,)).fetchall()
                return [dict(r) for r in rows]
    except Exception as e:
        logger.error(f"❌ get_trending_news: {e}")
        return []
...
def get_news(page: int = 1, per_page: int = 20, search: str = None, category: str = None):
    ...
    try:
        if USE_POSTGRES:
            ...
            cur.execute(f"SELECT * FROM posted_news{where_sql} ORDER BY posted_at DESC LIMIT %s OFFSET %s",
            ...
            with get_sqlite_conn() as conn:
                where, params = [], []
                if search:
                    where.append("title LIKE ?"); params.append(f"%{search}%")
                if category:
                    where.append("category = ?"); params.append(category)
                where_sql = (" WHERE " + " AND ".join(where)) if where else ""
                rows = conn.execute(
                    f"SELECT * FROM posted_news{where_sql} ORDER BY posted_at DESC LIMIT ? OFFSET ?",
                    tuple(params + [per_page, offset])
                ).fetchall()
...
def get_stats():
    ...
    try:
        if USE_POSTGRES:
            ...
            cur.execute("SELECT source, COUNT(*) as c FROM posted_news GROUP BY source ORDER BY c DESC LIMIT 10")
            ...
            with get_sqlite_conn() as conn:
                ...
                sources = [{"name": r[0], "count": r[1]} for r in conn.execute(
                    "SELECT source, COUNT(*) as c FROM posted_news GROUP BY source ORDER BY c DESC LIMIT 10").fetchall()]
