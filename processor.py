...
def format_message(item: dict):
    ...
    # إذا ما عطاش AI summary، نخدم RSS summary
    if not ai["summary"] and item.get("summary"):
        ai["summary"] = item["summary"][:200]
    ...
    if ai["summary"]:
        ...
        if ai.get("sentiment"):
            msg += f"├ {safe_html(ai['sentiment'])}\n"
        if ai.get("reason"):
            msg += f"└ {safe_html(ai['reason'])}\n"
        msg += "\n"
